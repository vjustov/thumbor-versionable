class Versionable::Version
  ...
  def calculate_metadata
    {
      width: @width = width != 0 ? width : ( image.width * height )/ image.height,
      height: @height = height != 0 ? height : ( image.height * width )/ image.width
    }
  end
  ...
  def sign
    secret_key =  Versionable.config.secret_key
    Base64.urlsafe_encode64(OpenSSL::HMAC.digest('sha1', secret_key, options_url))
  end
  ...
  def filters
    @filters.inject("filters") do |result, filter|
      result = result + ":#{filter_name(filter)}(#{args_from filter_params(filter)})"
